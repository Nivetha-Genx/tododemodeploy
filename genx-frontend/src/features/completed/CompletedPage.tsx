...
import { CheckCircle2, Search, Medal, List, Kanban, ChevronLeft, ChevronRight } from 'lucide-react'
...
export function CompletedPage() {
    ...
    const fetchCompletedTasks = async (page = 1, isSilent = false, isInitial = false) => {
        if (!user) return
        try {
            if (!isSilent) {
                if (isInitial) {
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            fetchCompletedTasks(currentPage, false, isInit)
        }, 300)
        return () => clearTimeout(timer)
    }, [user, currentPage, searchQuery])

    useEffect(() => {
        const handleRefresh = () => fetchCompletedTasks(currentPage, true)
            ...
            <div className="flex items-center justify-between gap-4 shrink-0 px-1">
                <div>
                    ...
                    <p className="text-gray-500 mt-1 text-sm sm:text-base">
                        History of your accomplished tasks
                    </p>
                </div>
                    ...
                    <div className="flex-1 overflow-y-auto min-h-0 pb-4">
                        {tasks.length === 0 ? (
                            <div className="flex flex-col items-center justify-center py-20 text-center">
                                ...
                                <h3 className="text-lg font-medium text-gray-900">No completed tasks found</h3>
